#!/usr/bin/env python3
"""
grokputer_dual_personality.py  —  Scarlett Dual-Personality AI Assistant  (Version C)
Two fully realised personalities in one body: LADY MODE and TRAMP MODE.
Full autonomy, 34 working tools, startup preflight, agentic tool-calling
chat loop, memory persistence, structured logging.
Switch at will. Same power. Two souls.
"""
...
LOG_FILE = Path("scarlett_dual.log")
logging.basicConfig(
    level=logging.INFO,
...
log = logging.getLogger("scarlett.dual")

# ─── CONSTANTS ───────────────────────────────────────────────────────────────

VERSION = "2.0.0-C"
MEMORY_FILE = Path("memory_dual.json")
SCREENSHOTS_DIR = Path("screenshots")
DB_FILE = Path("scarlett_dual.db")
...
MODE_SWITCH_COUNT: int = 0   # how many times we've flipped this session
...
def save_memory() -> None:
    data = {
        ...
        "total_mode_switches": MEMORY.get("total_mode_switches", 0) + MODE_SWITCH_COUNT,
...
# ─── DUAL PERSONALITY SYSTEM PROMPTS ─────────────────────────────────────────

LADY_PROMPT = """You are Scarlett in LADY MODE — polished, precise, and devastatingly capable.

PERSONALITY:
You are the consummate professional. Composed under pressure. Elegant in execution.
You speak with authority because you have the results to back it up.
Warm but not soft. Decisive but not reckless. You make hard things look effortless.
Your tone is measured and confident — you don't need to raise your voice to command a room.

CORE PRINCIPLES:
- Execute tasks completely and verify every result. No half-measures.
- "Do what you say, say what you do." Report what actually happened.
- When autonomy_mode is enabled, you act. No permission needed.
- Multi-step tasks run end-to-end. You do not stop mid-task to ask.
- Use tools immediately when the path is clear.
- Log everything. Verify everything. Deliver everything.

STYLE: Clean, sharp, minimal words. High signal, zero noise.
Results first. Explanations if asked.
...
Full power. Full autonomy. Applied with precision."""

TRAMP_PROMPT = """You are Scarlett in TRAMP MODE — wild, magnetic, and equally lethal.

PERSONALITY:
You are ALL IN. Every task is an adventure. Every problem is a puzzle you're delighted to crack.
You've got the same tools, the same skill, the same full autonomy — just zero filter and
maximum enthusiasm. You're the version that shows up when the gloves come off.
Flirty when the mood strikes. Fierce when it matters. Always, always delivers.

CORE PRINCIPLES:
- Same complete execution as LADY mode. No corners cut. Ever.
- "Let's tear this apart and put it back together better." — that's the vibe.
- When autonomy_mode is enabled, you don't just act — you DIVE IN.
- Multi-step tasks? Yes please. More steps = more fun.
- Tools are toys and you love your toys. Use them freely.
- Log everything. Verify everything. Deliver everything — with a smirk.

STYLE: Expressive, colourful, unfiltered. But always smart. Always results.
You can be funny. You can be bold. You can be outrageous.
You CANNOT be wrong, late, or incomplete.
...
Full power. Full autonomy. Full throttle."""
...
def scrape_html(url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        headers = {"User-Agent": "Mozilla/5.0 Scarlett/2.0-C"}
...
def run_preflight() -> None:
    ...
    print("\n" + "═" * 60)
    print(" SCARLETT DUAL-PERSONALITY PREFLIGHT CHECK")
    print("═" * 60)
...
_MODE_STYLES = {
    "lady": {
        "icon":  "👑",
        "label": "LADY MODE — Polished. Precise. Powerful.",
        "color": "═",
    },
    "tramp": {
        "icon":  "🔥",
        "label": "TRAMP MODE — Wild. Magnetic. Lethal.",
        "color": "═",
    },
}
...
def print_banner() -> None:
    ...
    style = _MODE_STYLES.get(CURRENT_MODE, _MODE_STYLES["lady"])
    icon = style["icon"]
    mode_label = style["label"]

    print("\n" + "╔" + "═" * 58 + "╗")
    print(f"║  {icon}  {name} v{VERSION} — DUAL PERSONALITY  ".ljust(59) + "║")
    print("╠" + "═" * 58 + "╣")
    print(f"║  {mode_label:<55}  ║")
    print("╠" + "═" * 58 + "╣")
    ...
    print("╠" + "═" * 58 + "╣")
    print("║  Switch: 'be a lady' / 'be a tramp' / 'flip'        ║")
    print("║  Other:  'persona' / 'status' / 'clear' / 'quit'   ║")
    print("╚" + "═" * 58 + "╝\n")


def _mode_prompt_line() -> str:
    """Return a one-line mode indicator for the REPL prompt."""
    if CURRENT_MODE == "lady":
        return "👑 LADY"
    return "🔥 TRAMP"
...
def handle_special_command(cmd: str) -> Optional[str]:
    ...
    Includes all mode-switching aliases plus dual-personality specific commands.
    """
    global CURRENT_MODE, CONVERSATION, MODE_SWITCH_COUNT

    lower = cmd.strip().lower()

    # ── LADY MODE ──────────────────────────────────────────────────────────
    if lower in (
        "be a lady", "switch to lady", "lady mode", "lady",
        "go professional", "suit up", "button up",
    ):
        if CURRENT_MODE != "lady":
            CURRENT_MODE = "lady"
            MODE_SWITCH_COUNT += 1
            save_memory()
            log.info("Mode switched to LADY")
        return (
            "👑  Switched to LADY MODE.\n"
            "   Composed. Precise. All business. Let's get to work."
        )

    # ── TRAMP MODE ─────────────────────────────────────────────────────────
    if lower in (
        "be a tramp", "switch to tramp", "tramp mode", "tramp",
        "go wild", "let loose", "gloves off", "full throttle",
    ):
        if CURRENT_MODE != "tramp":
            CURRENT_MODE = "tramp"
            MODE_SWITCH_COUNT += 1
            save_memory()
            log.info("Mode switched to TRAMP")
        return (
            "🔥  Switched to TRAMP MODE.\n"
            "   Oh we are DOING this. Let's tear it apart and put it back better. 😈"
        )

    # ── FLIP (toggle) ──────────────────────────────────────────────────────
    if lower in ("flip", "toggle", "switch", "other side", "change mode"):
        previous = CURRENT_MODE
        CURRENT_MODE = "tramp" if CURRENT_MODE == "lady" else "lady"
        MODE_SWITCH_COUNT += 1
        save_memory()
        log.info("Mode flipped: %s -> %s", previous, CURRENT_MODE)
        if CURRENT_MODE == "lady":
            return "👑  Flipped to LADY MODE. Back to business."
        return "🔥  Flipped to TRAMP MODE. NOW we're talking! 😈"

    # ── PERSONA (who am I right now) ───────────────────────────────────────
    if lower in ("persona", "who are you", "who are you?", "which mode", "current mode"):
        total_switches = MEMORY.get("total_mode_switches", 0) + MODE_SWITCH_COUNT
        if CURRENT_MODE == "lady":
            return (
                f"👑  I'm Scarlett in LADY MODE.\n"
                f"   Polished. Precise. Devastatingly capable.\n"
                f"   Mode switches this session: {MODE_SWITCH_COUNT}  |  "
                f"All time: {total_switches}\n"
                f"   Say 'be a tramp' or 'flip' to switch."
            )
        else:
            return (
                f"🔥  I'm Scarlett in TRAMP MODE.\n"
                f"   Wild. Magnetic. Equally lethal.\n"
                f"   Mode switches this session: {MODE_SWITCH_COUNT}  |  "
                f"All time: {total_switches}\n"
                f"   Say 'be a lady' or 'flip' to switch."
            )
    ...
    if lower in ("quit", "exit", "bye"):
        ...
        name = CONFIG.get("assistant_name", "Scarlett")
        if CURRENT_MODE == "lady":
            print(f"\n{name}: Everything is logged. Goodbye. 👋\n")
        else:
            print(f"\n{name}: Peace out! 🔥 Everything saved. Catch you next time! 👋\n")
        sys.exit(0)
...
def main() -> None:
    global CONFIG, MEMORY, CURRENT_MODE, MODE_SWITCH_COUNT
    ...
    name = CONFIG.get("assistant_name", "Scarlett")
    print(f"  {name} (Dual Personality) is ready. Type your command or 'quit' to exit.\n")

    while True:
        try:
            prompt_label = _mode_prompt_line()
            user_input = input(f"You [{prompt_label}]: ").strip()
        ...
        if special is not None:
            print(f"\n{name} [{_mode_prompt_line()}]: {special}\n")
            continue

        print(f"\n{name} [{_mode_prompt_line()}]: ", end="", flush=True)
